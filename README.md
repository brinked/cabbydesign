...
- **Appliances** — grill, griddle, side-burner, power-burner, kamado, **fridge and ice-maker** cabinets each get an **appliance dropdown** in their settings popup: pick a model from the admin-managed **inventory** (brand, model #, MSRP), or choose **Customer's own** and type the appliance the customer is supplying. Grill cabinets also surface the grill's **recommended insulated liner** with a *with / without liner* toggle. Selected appliances appear in their own **Appliances** table on the report at MSRP, roll into the estimate total, and are taxed like cabinets.
  - **Refrigeration height gap** — fridge and ice-maker models carry their own **W×D×H** (set in admin). When a unit is **shorter than the cabinet**, it renders at its real height with the countertop still at cabinet height, so the **gap** is visible (e.g. a 32″ fridge in a 34.5″ cabinet shows a 2.5″ gap) — helping customers plan the install. The cabinet's Width minimum is also raised to the unit's width.
  - **Panel-ready fridges** — a model with a **panel charge** (admin-set $ per model) adds a separate "custom cabinet-matched panel" line on the report.
